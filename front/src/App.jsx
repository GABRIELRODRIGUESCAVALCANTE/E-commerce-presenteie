...
// ==========================================
// TELA DE LOGIN / CADASTRO COM VERIFICAÇÃO DE E-MAIL OTP
// ==========================================
const Autenticacao = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isLogin) {
      ...
      try {
        ...
        setStep('verify');
        setCooldown(60);
        showToast(`Código de 6 dígitos enviado para ${email}`, 'info');
      } catch (erro) {
        showToast(erro.response?.data?.error || erro.message || 'Erro ao enviar código de verificação.', 'error');
      } finally {
        setEnviando(false);
      }
    }
  ...
  const handleVerifyAndRegister = async (e) => {
    ...
    if (!/^\d{6}$/.test(cleanCode)) {
      showToast('Digite o código de 6 dígitos recebido por e-mail.', 'warning');
      return;
    }
    ...
    try {
      ...
      await axios.post(`${API}/auth/verify-code`, { email, code: cleanCode });
      ...
      showToast(erro.response?.data?.error || erro.message || 'Código incorreto ou expirado.', 'error');
    } finally {
      setEnviando(false);
    }
  };
  ...
  const handleResendCode = async () => {
    ...
    try {
      ...
      showToast(erro.response?.data?.error || 'Erro ao reenviar código.', 'error');
    } finally {
      setEnviando(false);
    }
  };
      ...
      {step === 'verify' && !isLogin ? (
        <div>
          ...
          <p className="auth-subtitle">
            Enviamos um código de 6 dígitos para <strong>{email}</strong>. Digite-o abaixo para concluir seu cadastro:
          </p>

          <form onSubmit={handleVerifyAndRegister} className="form-box">
            <input
              type="text"
              inputMode="numeric"
              maxLength={6}
            ...
            <button type="submit" className="btn btn-primary btn-block" disabled={enviando} style={{ marginTop: '1rem', padding: '0.9rem' }}>
              {enviando ? 'Verificando...' : 'Confirmar e Criar Conta'}
            </button>
          </form>
          ...
          <div style={{ marginTop: '1.5rem', textAlign: 'center' }}>
            <button
              ...
              style={{ color: cooldown > 0 ? 'var(--color-text-muted)' : 'var(--color-primary)' }}
            >
          ...
          >
            ← <span>Alterar dados ou e-mail</span>
          </p>
        </div>
      ) : (
        <div>
          ...
          <p className="auth-subtitle">
            {isLogin ? 'Acesse sua conta para acompanhar pedidos' : 'Cadastre-se e receba confirmações de pedidos por e-mail'}
          </p>
          ...
          <form onSubmit={handleSubmit} className="form-box">
            ...
            <input type="password" placeholder="Senha (mínimo 8 caracteres)" value={senha} onChange={(e) => setSenha(e.target.value)} required minLength={8} />
            <button type="submit" className="btn btn-primary btn-block" disabled={enviando}>
              {enviando ? 'Carregando...' : isLogin ? 'Entrar' : 'Continuar →'}
            </button>
          </form>
          <p className="auth-toggle" onClick={() => { setIsLogin(!isLogin); setStep('form'); }}>
            {isLogin ? (
    ...
    <div className="checkout-container">
      ...
      <div className="checkout-grid">
        <div className="checkout-form">
          <h3 style={{ marginBottom: '0.75rem', color: 'var(--color-primary-dark)' }}>Retirada no Local</h3>
          <p style={{ color: 'var(--color-text-muted)', marginBottom: '1.25rem', fontSize: '0.95rem' }}>
            Estamos trabalhando exclusivamente com retirada no local. O pagamento é realizado diretamente na retirada do produto.
          </p>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
              <div>
                <strong style={{ display: 'block', marginBottom: '0.25rem', color: 'var(--color-text)' }}>Local de Retirada:</strong>
                <p style={{ margin: 0, fontSize: '0.9rem', color: 'var(--color-text-muted)', lineHeight: '1.4' }}>
                  Acesse a rota exata no mapa para retirar seu presente:
                </p>
                <a
          ...
          {pedidos.map((pedido) => (
            <div key={pedido.id} className="pedido-card">
              ...
              <p style={{ fontSize: '0.9rem', color: 'var(--color-text-muted)', lineHeight: '1.5' }}>
                <strong>{pedido.metodo_entrega === 'retirada' || pedido.endereco === 'Retirada' ? '📍 Retirada no Local:' : '🚚 Entrega:'}</strong>{' '}
                {pedido.metodo_entrega === 'retirada' || pedido.endereco === 'Retirada' ? (
                  <a
                    href="https://maps.app.goo.gl/bJLVbX5qwyjFf67A9"
                    target="_blank"
                    rel="noopener noreferrer"
                    style={{ color: 'var(--color-primary)', fontWeight: 600, textDecoration: 'underline', marginLeft: '4px' }}
                  >
                    Ver endereço no Google Maps ↗
                  </a>
                ) : (
                  `${pedido.endereco}, ${pedido.cidade} — CEP: ${pedido.cep}`
                )}
              </p>
...
const RotaProtegida = ({ children, session, isPending }) => {
  ...
  if (!emailAdmin || session.user.email.toLowerCase() !== emailAdmin.toLowerCase()) {
